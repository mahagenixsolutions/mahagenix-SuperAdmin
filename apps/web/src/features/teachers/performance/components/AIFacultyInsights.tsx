...
import type { AIFacultyInsight } from '../types';
import { Sparkles, Award, AlertTriangle, ArrowRight } from 'lucide-react';

interface AIFacultyInsightsProps {
  insights: AIFacultyInsight[];
  onTakeAction: (insight: AIFacultyInsight) => void;
}

export const AIFacultyInsights: React.FC<AIFacultyInsightsProps> = ({ insights, onTakeAction }) => {
  return (
    <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            <h2 style={{ margin: 0, fontSize: '18px', fontWeight: 800, color: '#0F172A' }}>
              EduVerse AI Faculty Intelligence & Award Nominations
            </h2>
            <p style={{ margin: '2px 0 0 0', fontSize: '13px', color: '#64748B' }}>
              Pattern analysis identifying top-performing teachers, syllabus lag warnings, and peer mentoring needs.
            </p>
          </div>
        </div>
      </div>
          ...
          >
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{
                  background: item.severity === 'Success' ? '#ECFDF5' : item.severity === 'Critical' ? '#FEF2F2' : '#FFFBEB',
                  color: item.severity === 'Success' ? '#047857' : item.severity === 'Critical' ? '#DC2626' : '#B45309',
                  border: `1px solid ${item.severity === 'Success' ? '#A7F3D0' : item.severity === 'Critical' ? '#FCA5A5' : '#FDE68A'}`,
                  padding: '2px 8px',
                  borderRadius: '12px',
                  fontSize: '10px',
                  fontWeight: 800
