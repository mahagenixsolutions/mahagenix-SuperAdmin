...
import type { NoticeAlert } from '../types';
...
export const RightSidebarAlerts: React.FC<RightSidebarAlertsProps> = ({ alerts, onResolveAlert }) => {
  const getAlertIcon = (type: NoticeAlert['type']) => {
    switch (type) {
      case 'High Priority Circular':
        return <Bell size={15} color="#3B7E5E" />;
      case 'Exam Notice Pending':
