...
export const AIInsightsPanel: React.FC<AIInsightsPanelProps> = ({ insights, onTakeAction }) => {
  const getSeverityStyle = (severity: AIInsight['severity']) => {
    switch (severity) {
      case 'Critical':
        return { bg: '#FEF2F2', border: '#FCA5A5', color: '#991B1B', iconColor: '#EF4444' };
      case 'Warning':
        return { bg: '#FEF3C7', border: '#FDE68A', color: '#92400E', iconColor: '#F59E0B' };
      case 'Info':
        return { bg: '#EFF6FF', border: '#BFDBFE', color: '#1E40AF', iconColor: '#3B82F6' };
    }
  };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        marginBottom: '20px',
        paddingBottom: '16px',
        borderBottom: '1px solid rgba(95, 175, 136, 0.2)'
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            <h2 style={{ margin: 0, fontSize: '18px', fontWeight: 800, color: '#0F172A', display: 'flex', alignItems: 'center', gap: '8px' }}>
              EduVerse AI Supervisory Insights
              <span style={{
                background: '#EAF5F0',
                color: '#3B7E5E',
                fontSize: '11px',
            ...
            <p style={{ margin: '2px 0 0 0', fontSize: '13px', color: '#64748B' }}>
              Automated pattern detection for attendance drop-offs, schedule overruns, late starts, and teacher overload.
            </p>
          </div>
        </div>

        <div style={{ fontSize: '12px', fontWeight: 700, color: '#3B7E5E', display: 'flex', alignItems: 'center', gap: '6px' }}>
          <Lightbulb size={16} /> 4 Smart Recommendations Active
        </div>
      </div>
